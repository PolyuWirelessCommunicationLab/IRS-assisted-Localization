...
K = 3;   %%Number of users
L = 6;  %%Number of elements of the virtual array in (17) and (18)
...
for mtc = 1:MTC
    ...
    lambda = 3*10^8/(2.4*10^9); %%Wavelength
 ...
 %%%%%%%%Generate user locations
    ...
    beta = [];
    G_tx = 5; %antenna gain of the user
    G_rx = 10;%antenna gain of the BS
    G_irs = 5; %gain of the IRS
    for k=1:K
        %Type I path loss model: Intelligent Reflecting Surface vs. Decode-and-Forward: How Large Surfaces Are Needed to Beat Relaying?
        beta_k = sqrt(10^((G_rx+G_irs-28-20*log10(fc)-22*log10(d_BI))/10))*sqrt(10^((G_rx+G_irs-28-20*log10(fc)-22*log10(d_UI(k)))/10))*exp(sqrt(-1)*randn(1,1));
        %Type II path loss model: free space path loss model without considering the antenna gain
        %beta_k = (lambda/sqrt(4*pi)/d_BI)*(lambda/sqrt(4*pi)/d_UI(k))*exp(sqrt(-1)*randn(1,1)); %%product of the IRS-BS path and user-IRS path
